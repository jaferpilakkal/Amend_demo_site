...
const demoDoctors = [
  ['Dr. Asha Menon', 'Endodontics & Restorative Care', 'BDS, MDS · fictional demo profile', 'Focused on clear explanations, careful treatment planning, and helping anxious patients feel at ease.'],
  ['Dr. Nikhil Thomas', 'Implant & Prosthetic Dentistry', 'BDS, MDS · fictional demo profile', 'Brings a detail-led approach to restoring comfort, function, and natural-looking smiles.'],
  ['Dr. Meera Nair', 'Orthodontics & Family Dentistry', 'BDS, MDS · fictional demo profile', 'Believes good dental experiences begin with listening, especially for children and first-time patients.'],
];
document.querySelector('#team-profiles').innerHTML = demoDoctors.map((doctor, index) => `<article class="doctor"><div class="portrait-placeholder"><span class="portrait-number">0${index + 1}</span><i data-lucide="user-round"></i><span>Demo portrait</span></div><h3>${doctor[0]}</h3><p class="doctor-role">${doctor[1]}</p><p>${doctor[2]}</p><p class="doctor-bio">${doctor[3]}</p></article>`).join('');
const demoCases = [
  ['Conservative Smile Refinement', 'Uneven edges and small spaces affected smile confidence.', 'A sample plan using conservative restorative options after assessment.', 'A balanced, natural-looking direction developed around the patient’s preferences.'],
  ['Single-Tooth Restoration', 'A damaged tooth caused discomfort while chewing.', 'A sample diagnostic and restorative pathway focused on preserving healthy tooth structure.', 'Comfort and function reviewed through ongoing follow-up.'],
];
document.querySelector('#cases').innerHTML = demoCases.map((item, number) => `<article class="case"><div class="comparison" style="--position:50%"><div class="comparison-layer after"><span>AFTER</span><p>Demo image area<small>No clinical result shown</small></p></div><div class="comparison-layer before"><span>BEFORE</span><p>Demo image area<small>No patient photography used</small></p></div><div class="comparison-handle"><i data-lucide="move-horizontal"></i></div><input type="range" min="0" max="100" value="50" aria-label="Demo before and after comparison for case ${number + 1}" aria-valuetext="50% before image" /></div><div class="case-info"><p class="eyebrow">CASE 0${number + 1} · FICTIONAL DEMO</p><h3>${item[0]}</h3><dl><div><dt>Concern</dt><dd>${item[1]}</dd></div><div><dt>Approach</dt><dd>${item[2]}</dd></div><div><dt>Outcome</dt><dd>${item[3]}</dd></div></dl></div></article>`).join('');
